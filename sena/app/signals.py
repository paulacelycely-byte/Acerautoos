            ...
            <table cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td style="vertical-align:middle;">
                  <h1 style="margin:0;color:#fff;font-size:1.1rem;
    ...
    if restantes <= 0:
        ...
        if not existe:
            ...
            filas = (
                ...
                + _fila("Km actuales est.", f"{vehiculo.km_estimados_hoy():,} km")
                + _fila("Km programado",    f"{vehiculo.km_proximo_mantenimiento:,} km")
                + _fila("Km excedidos",     f"{abs(restantes):,} km", ultimo=True)
            )
            pie = """
            <p style="margin:20px 0 0;padding:14px 16px;background:#fff5f5;
            ...
            enviar_correo_html(
                ...
                texto_plano = f"El vehiculo {vehiculo.placa} tiene mantenimiento vencido por {abs(restantes):,} km.",
                html        = html,
            )
        ...
        if not existe:
            Notificacion.objects.create(
                ...
                mensaje  = (
                    ...
                    f"Le quedan aproximadamente {restantes:,} km. "
                    f"Km programado: {vehiculo.km_proximo_mantenimiento:,}."
                ),
            )
            filas = (
                ...
                + _fila("Km actuales est.", f"{vehiculo.km_estimados_hoy():,} km")
                + _fila("Km programado",    f"{vehiculo.km_proximo_mantenimiento:,} km")
                + _fila("Km restantes",     f"~{restantes:,} km", ultimo=True)
            )
            pie = """
            ...
            enviar_correo_html(
                ...
                texto_plano = f"El vehiculo {vehiculo.placa} tiene ~{restantes:,} km para su proximo mantenimiento.",
                html        = html,
            )