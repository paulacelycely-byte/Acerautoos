...
def generar_notificaciones_automaticas():
    ...
    de cambio de aceite basadas en km y fecha según el tipo de uso.
    Envía correo SOLO para notificaciones de cambio de aceite.
    """
    for v in Vehiculo.objects.select_related('marca', 'cliente').all():
        ...
        # ── Siempre es cambio de aceite (mantenimiento principal del vehículo) ──
        tipo   = 'Mantenimiento'
        titulo = f'Cambio de aceite próximo — {v.placa}'
        partes = []
        ...
        if dias_rest is not None:
            if dias_rest <= 0:
                ...
                partes.append(
                    f'El cambio de aceite venció hace {dias_abs} día{"s" if dias_abs != 1 else ""}.'
                )
                tipo   = 'Urgente'
                titulo = f'Cambio de aceite vencido — {v.placa}'
            elif dias_rest <= 7:
                partes.append(
                    f'Faltan {dias_rest} día{"s" if dias_rest != 1 else ""} para el cambio de aceite '
                    f'(fecha límite: {v.fecha_proximo_mantenimiento.strftime("%d/%m/%Y")}).'
                )
                tipo = 'Urgente'
            elif dias_rest <= 15:
                partes.append(
                    f'Faltan {dias_rest} días para el cambio de aceite '
                    f'(fecha límite: {v.fecha_proximo_mantenimiento.strftime("%d/%m/%Y")}).'
                )
                tipo = 'Alerta'
            elif dias_rest <= 30:
                partes.append(
                    f'Falta aproximadamente 1 mes para el cambio de aceite '
                    f'(fecha límite: {v.fecha_proximo_mantenimiento.strftime("%d/%m/%Y")}).'
                )
            elif dias_rest <= 60:
                meses = round(dias_rest / 30)
                partes.append(
                    f'Faltan aproximadamente {meses} meses para el cambio de aceite '
                    f'(fecha límite: {v.fecha_proximo_mantenimiento.strftime("%d/%m/%Y")}).'
                )
        ...
        if km_rest is not None:
            if km_rest <= 0:
                partes.append(
                    f'El vehículo superó el límite de km para cambio de aceite '
                    f'(excedido por {abs(int(km_rest)):,} km). '
                    f'Uso del vehículo: {v.get_tipo_uso_display()} (~{v.km_diarios()} km/día).'
                )
                tipo   = 'Urgente'
                titulo = f'Cambio de aceite vencido — {v.placa}'
            elif km_rest <= v.km_alerta_anticipacion:
                partes.append(
                    f'Faltan {int(km_rest):,} km para el cambio de aceite '
        ...
        if not ya_existe:
            ...
            # ── Solo enviar correo si es cambio de aceite ──
            if 'aceite' in titulo.lower() or 'aceite' in mensaje.lower():
...
class NotificacionCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        # ── Solo enviar correo si es cambio de aceite ──
        titulo_lower  = (notificacion.titulo  or '').lower()
        mensaje_lower = (notificacion.mensaje or '').lower()
        es_cambio_aceite = 'aceite' in titulo_lower or 'aceite' in mensaje_lower
 
        if notificacion.vehiculo and notificacion.vehiculo.cliente and es_cambio_aceite:
